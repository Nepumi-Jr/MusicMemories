...
--[[ for indx=1,iTilesX do
	for indy=1,iTilesY do
		t[#t+1] = Def.Quad {
			...
			OnCommand=function(self) self:diffuse(Color("Black")); self:diffusealpha(1); self:zoom(fTileSize-2); self:sleep((iTilesX+iTilesY)*(1/60)); self:linear(0.0325 + ( indx / 60 )); self:diffusealpha(0); self:zoom(fTileSize*1.25); end;
		};
	end
end --]]
t[#t+1] = Def.Quad {
	InitCommand=function(self) self:zoomto(SCREEN_WIDTH+1,SCREEN_HEIGHT); end;
	OnCommand=function(self) self:diffuse(color("0,0,0,1")); self:sleep(0.0325 + fSleepTime); self:linear(0.15); self:diffusealpha(0); end;
};
