...
t[#t+1] = Def.ActorFrame {
  InitCommand=function(self) self:x(SCREEN_CENTER_X); self:y(SCREEN_CENTER_Y); end;
	LoadFont("Common Normal") .. {
		...
		BeginCommand=function(self) self:visible(SCREENMAN:GetTopScreen():GetAnyEntering()); end;
	};
	LoadFont("Common Normal") .. {
		Text="No players has a high score";
		BeginCommand=function(self) self:visible(not SCREENMAN:GetTopScreen():GetAnyEntering()); end;
	};
--[[ 	for pn in ivalues(Players) do
		LoadFont("Common Normal") .. {
			Text=ToEnumShortString(pn);
			InitCommand=function(self) self:x(128 * tMath[pn]); self:y(32); end;
		};
	end --]]
};
--
