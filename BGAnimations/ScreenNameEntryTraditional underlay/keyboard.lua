...
local letter = LoadFont("Common Normal")..{
	InitCommand=function(self) self:zoom(0.8); self:shadowlength(0); end;
	PulseCommand = function(self)
		self:finishtweening();
		local z = self:GetZoomX();
		(function(self) self:accelerate(0.15); self:zoom(1.2); self:decelerate(0.15); self:zoom(z); end)(self);
	end;
};
...
for pn in ivalues(PlayerNumber) do
	children[#children+1] = LoadActor( THEME:GetPathG("_frame", "1D"),
		...
		Name = CursorFiles[pn];
		BeginCommand = function(self) self:visible(SCREENMAN:GetTopScreen():GetEnteringName(pn)); end;
		OnCommand = function(self) self:zoom(0); self:rotationz(-360*2); self:sleep(0.55); self:decelerate(0.5); self:zoom(1); self:rotationz(0); end;
		OffCommand = function(self) self:sleep(0.3); self:queuecommand("TweenOff"); end;
		...
		TweenOffCommand = function(self,param)
			(function(self) self:accelerate(0.25); self:zoomx(0); end)(self);
		end;
	};
end
...
return Def.ActorFrame {
	...
	SelectKeyMessageCommand = function(self, param)
		...
		c.Cursors[pn]:playcommand( "SetSize", { Width=Keys[key].Width+6; tween=function(self) self:stoptweening(); self:linear(0.10); end; } );
	...
	OnCommand = function(self, param)
		for key in ivalues(Letters) do
			...
			local f = function(self) self:diffusealpha(0); self:zoom(0); self:sleep(0.25 + (fDist / 400)); self:decelerate(0.5); self:diffusealpha(1); self:zoom(0.8); end;
			f(Keys[key].Text);
		end
	end;
	...
	OffCommand = function(self, param)
		for key in ivalues(Letters) do
			local fDist = distance( Keys[key].Text:GetX(), Keys[key].Text:GetY(), 0, 100 );
			local f = function(self) self:sleep(0.0 + (fDist / 400)); self:decelerate(0.5); self:diffusealpha(0); self:zoom(0); end;
			f(Keys[key].Text);
		end
	end;
};
