...
local screenVibrate = function(self)
	...
	if RSTR > 0 then
		...
		SCREENMAN:GetTopScreen():x(0):y(0)
	end
end;

local actorVibrate = function(self)
	local RSTR = math.ceil( STR )
	local Force = 0.3;
	if RSTR > 0 then
		self:x(math.random(-RSTR,RSTR)*Force);
		...
		self:x(0):y(0):zoom(1):rotationz(0)
	end
end;
...
local t = Def.ActorFrame{
	...
	Def.ActorFrame{
		OnCommand=function(self) self:SetUpdateFunction(screenVibrate); end;
	};
	...
	Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			Def.ActorFrame{
				OnCommand=function(self) self:SetUpdateFunction(actorVibrate); end;
				LoadFont("Common Normal")..{
