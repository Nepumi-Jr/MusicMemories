local function Clock()
	...
	clock = Def.ActorFrame {
		Name="Clock",
		InitCommand=function(self) self:x(50); self:y(12); self:playcommand("Update"); end,
		...
		Def.RoundedBox(90,26)..{ InitCommand=function(self) self:x(-22); self:y(-4); end },
		Def.ActorFrame {
			Name="ClockText",
			InitCommand=function(self) self:y(-2); end,
			LoadFont("Common", "normal")..{
				Text="00:00:",
				InitCommand=function(self) self:horizalign(right); self:shadowlength(0); self:diffusebottomedge(color("0.9,0.9,0.9")); end,
			...
			LoadFont("Common", "normal")..{
				Text="00",
				InitCommand=function(self) self:horizalign(left); self:shadowlength(0); self:diffusebottomedge(color("0.9,0.9,0.9")); end,
				UpdateCommand=function(self)
					local sec = Second()
			...
			LoadFont("Common", "normal")..{
				Text="",
				InitCommand=function(self) self:x(28); self:y(-3); self:horizalign(left); self:shadowlength(0); self:diffusebottomedge(color("0.9,0.9,0.9")); self:visible(false); self:zoom(0.75); end,
				UpdateCommand=function(self)
