...
local InputDanP1 = function( event )
	...
	if event.type == "InputEventType_FirstPress" then
		...
		if event.button == "Left" and event.PlayerNumber == pn then
			Dan1Press[1] = 1
		end
		if event.button == "UpLeft" and event.PlayerNumber == pn then
			Dan1Press[5] = 1
		end
		if event.button == "Down" and event.PlayerNumber == pn then
			Dan1Press[2] = 1
		end
		if event.button == "Up" and event.PlayerNumber == pn then
			Dan1Press[3] = 1
		end
		if event.button == "Right" and event.PlayerNumber == pn then
			Dan1Press[4] = 1
		end
		if event.button == "UpRight" and event.PlayerNumber == pn then
			Dan1Press[6] = 1
		end
...
local Kayay = 0.55;
local Rew = {0.07,0.15} ;
local Dan1Cl = BoostColor((Color[TP[ToEnumShortString(pn)].ActiveModifiers.ComboColorstring] or Color.White),1.4);
local Dan1ClF = BoostColor((Color[TP[ToEnumShortString(pn)].ActiveModifiers.ComboColorstring] or Color.White),0.2);
local Dan1TF = {true,false}--{D,(LU,RU)}
local t = Def.ActorFrame{};
local CodePor;

if GAMESTATE:IsCourseMode() then
CodePor = GAMESTATE:GetCurrentTrail(pn);
else
CodePor = GAMESTATE:GetCurrentSteps(pn);
end
...
t[#t+1] = Def.ActorFrame{
	OnCommand=function(self)
		...
		self:x(pn == PLAYER_1 and 42 or SCREEN_RIGHT-42)
		self:y(400)
