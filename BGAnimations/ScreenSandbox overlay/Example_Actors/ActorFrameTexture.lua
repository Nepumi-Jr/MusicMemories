...
local Example3_Diffuse = { 1,1,1,1 } -- Note that with Green of 50%, there is still a white rectangle where the Green square intersect the background white square.
...
Examples[1] = Def.ActorFrame{
	Def.Quad{ InitCommand=function(self) self:FullScreen(); end }; -- a blank background.
	Def.ActorFrameTexture{
		...
		Def.ActorFrame{
			...
			Def.Quad{ InitCommand=function(self) self:zoom(50); self:diffuse(1,0,0,0.5); end };
			Def.Quad{ InitCommand=function(self) self:zoom(50); self:diffuse(0,1,0,0.5); self:x(64); self:y(64); end };
			Def.Quad{ InitCommand=function(self) self:zoom(50); self:diffuse(0,0,1,0.5); self:x(120); self:y(100); end };
		};
	};
	Def.Sprite{ 
		Texture="Example 1";
		InitCommand=function(self) self:Center(); self:sleep(1); self:queuecommand("Scroll"); end;
		ScrollCommand=function(self) self:texcoordvelocity(1,0.5); end; -- Scroll the texture. Texture manipulation is one of the things that makes AFT special.
	};
}

Examples[2] = Def.ActorFrame{
	Def.Quad{ InitCommand=function(self) self:FullScreen(); self:diffuse(0,0,0,1); end }; -- a blank background.
	Def.ActorFrameTexture{
		InitCommand=function(self)
		...
		Def.ActorFrame{
			Name = "Draw";
			InitCommand=function(self) self:spin(); self:x(64); self:y(64); end; -- Something has to change to demonstrate PreserveTexture.
			Def.Quad{ InitCommand=function(self) self:zoom(50); self:rainbow(); self:effectperiod(3); self:effectoffset(1); self:x(-40); self:y(-56); end };
			Def.Quad{ InitCommand=function(self) self:zoom(50); self:rainbow(); self:effectperiod(3); self:effectoffset(0); end };
			Def.Quad{ InitCommand=function(self) self:zoom(50); self:rainbow(); self:effectperiod(3); self:effectoffset(2); self:x(64); self:y(50); end };
		};
	};
	Def.Sprite{ Texture="Example 2"; InitCommand=function(self) self:Center(); end; };
}
...
Examples[3] = Def.ActorFrame{
	...
	Def.ActorFrameTexture{
		...
		Def.ActorFrame{
			Name = "Draw";
			Def.Quad{ InitCommand=function(self) self:zoom(80); self:diffuse(1,1,1,1); self:x(64); self:y(64); end };
			Def.Quad{ InitCommand=function(self) self:zoom(80); self:diffuse(1,0,0,1); self:x(14); self:y(80); self:blend("BlendMode_Add"); end };
			Def.Quad{ InitCommand=function(self) self:zoom(80); self:diffuse(0,1,0,1); self:x(74); self:y(20); self:blend("BlendMode_Add"); end };
			Def.Quad{ InitCommand=function(self) self:zoom(80); self:diffuse(0,0,1,1); self:x(128); self:y(100); self:blend("BlendMode_Add"); end };
		};
	};
	Def.Sprite{ Texture = "Example 3"; InitCommand=function(self) self:Center(); self:diffuse(Example3_Diffuse); end; };
}
...
Examples[4] = Def.ActorFrame{
	...
	Def.ActorFrameTexture{
		...
		Def.Sprite{ Name = "Sprite"; InitCommand=function(self) self:x(128); self:y(128); end };
	};
	Def.ActorFrameTexture{
		InitCommand=function(self)
		...
		Def.Sprite{	Texture = "Memory"; InitCommand=function(self) self:x(128); self:y(128); self:diffuse(1,1,1,.995); end; };
		Def.ActorFrame{ -- eliptical motion.
			InitCommand=function(self) self:x(128); self:y(128); self:bob(); self:effectmagnitude(96,0,0); self:effectoffset(0.5); end;
			Def.ActorFrame{
				InitCommand=function(self) self:bob(); self:effectmagnitude(0,64,0); end;
				-- A pixel ghost.
				Def.Quad{ InitCommand=function(self) self:zoomto(32,40); end; };
				Def.Quad{ InitCommand=function(self) self:zoomto(8,8); self:x(-8); self:y(-8); self:diffuse(0,0,0,1); end; };
				Def.Quad{ InitCommand=function(self) self:zoomto(8,8); self:x(8); self:y(-8); self:diffuse(0,0,0,1); end; };
				Def.Quad{ InitCommand=function(self) self:zoomto(24,8); self:x(0); self:y(2); self:diffuse(0,0,0,1); end; };
				Def.Quad{ InitCommand=function(self) self:zoomto(8,16); self:x(-12); self:y(24); end; };
				Def.Quad{ InitCommand=function(self) self:zoomto(8,16); self:x(12); self:y(24); end; };
				Def.Quad{ InitCommand=function(self) self:zoomto(8,16); self:x(0); self:y(24); end; };
			};
		};
	};
	Def.Sprite{ Name = "Ghosting"; Texture = "Output"; InitCommand=function(self) self:Center(); end; };
}
