...
-- 			self:setaux(1);
	...
	Def.Quad {
		InitCommand=function(self) self:y(-14); self:zoomto(164,2); self:fadeleft(8/164); self:faderight(8/164); end;
		OnCommand=function(self) self:diffuse(Color("Black")); self:diffusealpha(0); self:linear(0.35); self:diffusealpha(0.5); end;
	};
	Def.Quad {
		InitCommand=function(self) self:y(24*(5)-10); self:zoomto(164,2); self:fadeleft(8/164); self:faderight(8/164); end;
		OnCommand=function(self) self:diffuse(Color("Black")); self:diffusealpha(0); self:linear(0.35); self:diffusealpha(0.5); end;
	};
	...
	Def.StepsDisplayList {
		...
		CursorP1 = Def.ActorFrame {
			InitCommand=function(self) self:x(-128+16); self:player(PLAYER_1); end;
			PlayerJoinedMessageCommand=function(self, params)
				if params.Player == PLAYER_1 then
					self:visible(true);
					(function(self) self:zoom(0); self:bounceend(1); self:zoom(1); end)(self);
				end;
			end;
			PlayerUnjoinedMessageCommand=function(self, params)
				if params.Player == PLAYER_1 then
					self:visible(true);
					(function(self) self:bouncebegin(1); self:zoom(0); end)(self);
				end;
			end;
		...
		CursorP2 = Def.ActorFrame {
			InitCommand=function(self) self:x(-128+16); self:player(PLAYER_2); end;
			PlayerJoinedMessageCommand=function(self, params)
		...
		CursorP1Frame = Def.Actor{
			ChangeCommand=function(self) self:stoptweening(); self:decelerate(0.05); end;
		};
